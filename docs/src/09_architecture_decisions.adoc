...
Consequences::
- Makes the client/server split explicit: “clients use public API”, “user management stays server-side”.
- Allows different security policies per boundary (API key for public game API, JWT for user/auth API).

IMPORTANT: Status: PROPOSED 

=== ADR-012: Reverse proxy to expose one client/server endpoint (Nginx)

Context::
- We want one public host/port while serving the client and routing requests to the server APIs.
- A full API gateway is unnecessary for the project scope.

Decision::
- Use Nginx for path-based routing:
+ / -> frontend (client)
+ /api/public/v1/ -> Node/Express (server: public game API)
+ /api/server/v1/ -> Node/Express (server: user/auth API)
+ (optional) /ws/ -> Node/Express (server: WebSocket for realtime/multiplayer)
