// webapp/src/components/stats/WinrateChart.tsx
...
export function WinrateChart({ data, title }: WinrateChartProps) {
  ...
  const lossLargeArc = lossAngle > 180 ? 1 : 0
  const lossEnd = toXY(0)
  const lossStart = toXY(winAngle)
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <svg width="120" height="120" viewBox="0 0 120 120">
        {data.losses > 0 && (
          <path d={lossPath} fill="hsl(var(--player2))" opacity="0.8" />
        )}
        {data.wins > 0 && (
          <path d={winPath} fill="hsl(var(--player1))" opacity="0.9" />
        )}
        <text
          x={cx}
          y={cy + 5}
