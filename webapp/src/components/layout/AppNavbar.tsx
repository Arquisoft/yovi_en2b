...
export function AppNavbar() {
  ...
  return (
    <>
      ...
      {/* ── Mobile menu — fixed so it stays visible while scrolling ─────────── */}
      {menuOpen && (
        <div className="sm:hidden fixed inset-x-0 top-14 z-40 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-4 py-2 space-y-0.5">
          {user && (
            <button
              onClick={() => !isGuest && go('/profile')}
              disabled={isGuest}
              className="flex items-center gap-3 w-full px-3 py-3.5 rounded-lg text-sm text-left hover:bg-muted/80 transition-colors disabled:opacity-50"
            >
          ...
          <button
            onClick={toggleTheme}
            className="flex items-center gap-3 w-full px-3 py-3.5 rounded-lg text-sm text-left hover:bg-muted/80 transition-colors"
          >
            {theme === 'dark'
          ...
          {user && (
            <>
              <button onClick={() => go('/stats')} className="flex items-center gap-3 w-full px-3 py-3.5 rounded-lg text-sm text-left hover:bg-muted/80 transition-colors">
                <BarChart2 className="h-4 w-4 shrink-0 text-muted-foreground" />
                <span>{t('nav.statistics')}</span>
              </button>

              <button onClick={() => go('/history')} className="flex items-center gap-3 w-full px-3 py-3.5 rounded-lg text-sm text-left hover:bg-muted/80 transition-colors">
                <Clock className="h-4 w-4 shrink-0 text-muted-foreground" />
                <span>{t('nav.history')}</span>
              </button>

              <button onClick={() => go('/ranking')} className="flex items-center gap-3 w-full px-3 py-3.5 rounded-lg text-sm text-left hover:bg-muted/80 transition-colors">
          ...
          <div className="flex items-center gap-2 px-3 py-3">
            ...
            <select
              value={locale}
              onChange={(e) => setLanguage(e.target.value as SupportedLocale)}
              className="bg-background/80 text-foreground border border-border rounded-md px-2 py-1 text-sm appearance-none"
            >
              {SUPPORTED_LOCALES.map((lang) => (
                <option key={lang} value={lang}>{lang.toUpperCase()}</option>
              ))}
          ...
          {user && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-3 w-full px-3 py-3.5 rounded-lg text-sm text-left hover:bg-muted/80 transition-colors text-destructive"
              >
