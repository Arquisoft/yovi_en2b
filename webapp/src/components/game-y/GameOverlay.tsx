...
export function getModeLabel(mode: GameMode): string {
  if (mode === 'pve')       return 'VS BOT'
  if (mode === 'pvp-local') return 'LOCAL'
  return 'ONLINE'
}

export function resolveResult(game: GameState, currentUserId: string): OverlayResult {
  const { winner, players, config } = game
  ...
  if (!winner) {
    return { isVictory: false, resultLabel: 'DRAW', winnerName: '', opponentName }
  }

  const winnerName = winner === 'player1' ? players.player1.name : players.player2.name

  if (isLocalGame) {
    return { isVictory: true, resultLabel: 'VICTORY', winnerName, opponentName }
  }
...
export function getAccentColor(isVictory: boolean, hasWinner: boolean): string {
  if (isVictory)  return PRIMARY_COLOR
  if (hasWinner)  return DESTRUCTIVE_COLOR
  return '#888'
}
...
export function ResultIcon({ isVictory, hasWinner }: ResultIconProps) {
  ...
  return <div style={{ fontSize: 48 }}>🤝</div>
}
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      particlesRef.current = particlesRef.current
        .map(p => ({ ...p, x: p.x + p.vx, y: p.y + p.vy, vy: p.vy + 0.25, vx: p.vx * 0.98, rotation: p.rotation + p.rotationSpeed, opacity: p.opacity - 0.007 }))
...
export function GameOverlay({ game, currentUserId, onPlayAgain, onGoHome }: GameOverlayProps) {
  ...
  return (
    <>
      <style>{`
        @keyframes go-fadein  { from{opacity:0} to{opacity:1} }
        @keyframes go-slam    { 0%{transform:scale(2.5);opacity:0;filter:blur(6px)} 65%{transform:scale(0.93);opacity:1;filter:blur(0)} 82%{transform:scale(1.04)} 100%{transform:scale(1)} }
        @keyframes go-rise    { from{transform:translateY(32px);opacity:0} to{transform:translateY(0);opacity:1} }
        @keyframes go-shimmer { 0%{background-position:-200% center} 100%{background-position:200% center} }
        @keyframes go-crown   { 0%,100%{transform:translateY(0) rotate(-5deg)} 50%{transform:translateY(-10px) rotate(5deg)} }
        @keyframes go-shake   { 0%,100%{transform:translateX(0)} 15%,45%,75%{transform:translateX(-5px)} 30%,60%,90%{transform:translateX(5px)} }
        @keyframes go-pulse-border {
          0%,100% { box-shadow:0 0 0 1.5px ${accentColor},0 0 24px ${accentColor}55,0 20px 60px rgba(0,0,0,.6) }
          50%     { box-shadow:0 0 0 1.5px ${accentColor},0 0 48px ${accentColor}99,0 20px 60px rgba(0,0,0,.6) }
        }
        .go-overlay { animation:go-fadein .35s ease both }
        .go-card    { animation:go-rise .45s cubic-bezier(.175,.885,.32,1.275) .05s both, go-pulse-border 2.5s ease .6s infinite }
        .go-crown   { animation:go-crown 2s ease-in-out infinite }
        .go-stats   { animation:go-rise .4s ease .2s both }
        .go-buttons { animation:go-rise .4s ease .3s both }
        .go-title-victory { background:linear-gradient(90deg,#FFD700,#4A9EFF,#FFD700,#fffbe0,#FFD700);background-size:200% auto;-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;animation:go-slam .55s cubic-bezier(.175,.885,.32,1.275) both,go-shimmer 2.5s linear .6s infinite }
        .go-title-defeat  { color:${DESTRUCTIVE_COLOR};text-shadow:0 0 24px ${DESTRUCTIVE_COLOR}99;animation:go-slam .55s cubic-bezier(.175,.885,.32,1.275) both,go-shake .45s ease .6s }
        .go-title-draw    { color:#888;animation:go-slam .55s cubic-bezier(.175,.885,.32,1.275) both }
        .go-btn-primary { background:${PRIMARY_COLOR};color:#fff;border:none;border-radius:8px;padding:13px 0;width:100%;font-size:15px;font-weight:700;letter-spacing:.03em;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;transition:filter .15s,transform .1s }
        .go-btn-primary:hover  { filter:brightness(1.15) }
        .go-btn-primary:active { transform:scale(.97) }
        .go-btn-ghost { background:transparent;color:#666;border:1px solid #2a2a2a;border-radius:8px;padding:12px 0;width:100%;font-size:14px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;margin-top:8px;transition:color .15s,border-color .15s }
        .go-btn-ghost:hover { color:#aaa;border-color:#444 }
        .go-btn-close { position:absolute;top:12px;right:12px;background:transparent;border:1px solid #2a2a2a;border-radius:6px;width:28px;height:28px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:#555;transition:color .15s,border-color .15s,background .15s }
