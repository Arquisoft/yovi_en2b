...
export function GameYPage() {
  ...
  const {
    ...
    isBotResolvingPie, swapAnimationStone, swapCommitted, opponentDisconnected, wsConnectionFailed,
  ...
  const pieDecisionStoneMobile = (isPieDecisionPending && !swapCommitted) || isSwapAnimating
    ? swapAnimationStone ?? lastMove : null
  ...
  const OpponentDisconnectedBanner = opponentDisconnected ? (
    <div className="flex items-center gap-2 px-3 py-2 bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 text-sm border-b border-yellow-500/20">
      <WifiOff className="w-4 h-4 shrink-0" />
      <span>{t('game.opponentDisconnected')}</span>
    </div>
  ) : null

  const ConnectionLostBanner = wsConnectionFailed ? (
  ...
  if (isMobile) {
    return (
      <div className="h-full min-h-0 flex flex-col relative">
        ...
        {moveError && (
          <div className="flex items-center gap-2 px-3 py-2 bg-destructive/10 text-destructive text-sm border-b border-destructive/20">
            <XCircle className="w-4 h-4 shrink-0" /><span>{moveError}</span>
          </div>
        )}
        <div className="flex-1 min-h-0 overflow-hidden relative">
          <GameYBoard game={game} lastMove={lastMove} isInteractive={canPlay && !isSwapAnimating}
            onCellClick={handleCellClick} pieDecisionStone={pieDecisionStoneMobile}
            isSwapAnimating={isSwapAnimating} swapCommitted={swapCommitted} />
          {showPiePanel && (
            <PieRuleDecisionPanel game={game} isBotDeciding={isBotDecidingPie || isBotResolvingPie}
    ...
    <div className="h-full min-h-0 flex flex-col relative">
      ...
      <div className="flex-1 min-h-0 flex relative">
        <div className="flex-1 min-w-0 overflow-hidden relative">
          <GameYBoard game={game} lastMove={lastMove} isInteractive={canPlay && !isSwapAnimating}
            onCellClick={handleCellClick} pieDecisionStone={pieDecisionStoneDesktop}
            isSwapAnimating={isSwapAnimating} swapCommitted={swapCommitted} />
          {showPiePanel && (
            <PieRuleDecisionPanel game={game} isBotDeciding={isBotDecidingPie || isBotResolvingPie}
              onDecide={handlePieDecision} isLoading={isPieDecisionLoading} />
        ...
        <div className="relative flex-shrink-0 flex">
          <button onClick={() => setSidebarOpen(o => !o)}
            className="absolute -left-4 top-1/2 -translate-y-1/2 z-10 w-4 h-12 flex items-center justify-center bg-card border border-border rounded-l-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
          ...
          <aside className="overflow-hidden border-l border-border bg-card transition-[width] duration-300 ease-in-out"
